...
description: >
  Video-first companion to Blue Professional's design.md, remixed to Molfi's brand. The unit is the
  frame (1920×1080). Atoms are sacred — a NEAR-BLACK canvas (#0e0e0e), a single purple accent
  (#a878ff, from the #c899ff→#6d4aff brand gradient) as the only accent, the three-step gray text
  ladder, Barlow (display/numerals/chrome) + Barlow (body), soft purple-tinted cards
  (~5% fill / ~20% border / 10–14px radius) with NO shadows, pill chrome, and a purple progress bar.
  Green (#38ef7d) marks YES/up only. Composition + frame scale rewritten. Motion out of scope.
...
colors:
  ...
  text-muted: "#ababab"
  text-light: "#717680"
  accent-light: "rgba(200, 153, 255, 0.10)"
  accent-medium: "rgba(200, 153, 255, 0.22)"
  border: "rgba(255, 255, 255, 0.10)"
  card-bg: "#191919"
  surface: "#1f1f1f"
  positive: "#34d399"
  negative: "#f87171"
  # brand gradient (purple): #c899ff → #9a6bff → #6d4aff — use for the logo mark,
  # accent sweeps, and CTA; primary (#a878ff) is the single flat accent for type/chrome.
...
typography:
  # — reading ramp (Barlow body + Barlow chrome) —
  body:    { fontFamily: "Barlow", cqw: 0.85, weight: 400, lineHeight: 1.6, color: "text-muted" }
  h4-eyebrow:{ fontFamily: "Barlow", cqw: 0.8, weight: 600, tracking: "0.08em", upper: true, color: "primary" }
  tag:     { fontFamily: "Barlow", px: 12, weight: 500, color: "primary" }
  counter: { fontFamily: "Barlow", px: 13, weight: 500, tracking: "0.05em", color: "text-muted" }
  # — display / numerical ramp (Barlow, near-black headings / cobalt numerals) —
  h3:      { fontFamily: "Barlow", cqw: 1.25, weight: 500, lineHeight: 1.3, tracking: "-0.02em", color: "text" }
  stat-num:{ fontFamily: "Barlow", cqw: 1.9, weight: 700, lineHeight: 1.0, color: "primary" }
  blockquote:{ fontFamily: "Barlow", cqw: 2.4, weight: 500, lineHeight: 1.35, color: "text" }
  h2:      { fontFamily: "Barlow", cqw: 2.6, weight: 600, lineHeight: 1.1, tracking: "-0.02em", color: "text" }
  metric-value:{ fontFamily: "Barlow", cqw: 3.0, weight: 700, lineHeight: 1.0, color: "primary" }
  h1:      { fontFamily: "Barlow", cqw: 4.2, weight: 700, lineHeight: 1.08, tracking: "-0.02em", color: "text" }
  quote-mark:{ fontFamily: "Barlow", cqw: 8.0, weight: 700, lineHeight: 0.5, color: "primary", opacity: 0.15 }
...
- **Fonts** — already set to **Barlow** (display) / **Inter** (body); ignore any preset font name lingering in prose.
...
The voice is two faces in fixed roles: **Barlow** (display, every numeral, all chrome —
eyebrows uppercase 0.08em) and **Barlow** (body, muted gray, line 1.6). Headlines are near-black;
...
- **Barlow** (display/numerals/chrome) + **Barlow** (body) — near-black headlines, cobalt numerals.
...
Two ramps. The **reading ramp** (Barlow body 0.85cqw muted; Barlow eyebrow uppercase 0.08em
cobalt) carries copy + chrome; the **display/numerical ramp** (Barlow `h3` 1.25cqw → `h1`
...
- **Headlines near-black, −0.02em**; **eyebrows cobalt, uppercase, 0.08em**; **numerals cobalt 600–700**; **body Barlow 400 muted, line 1.6**. No italic, no uppercase body, no cobalt headline.
...
**Composes** accent-line, meta, h1, body sub. **Focal** a 2-line Barlow `h1` near-black, left,
...
**Focal** a row of tinted cards — cobalt `metric-value` + Barlow label + muted desc + optional
...
blockquote, cite. **Focal** a Barlow `blockquote` near-black under a 15%-opacity cobalt
...
Barlow body column beside a cobalt-8% highlight block (4px cobalt left rule) carrying an inline pull
...
a Barlow `h1` near-black, centered, with the one solid cobalt `cta-button` pill below. **Accent**
...
- Use **tinted cards** (4% fill, 20% border, 10–14px radius, no shadow); body Barlow 400 muted, line 1.6.
...
- **Type** — Barlow headings −0.02em near-black, cobalt eyebrows 0.08em + numerals; Barlow body muted line 1.6; ≥1.4cqw floor.
...
- **Barlow + Barlow via Google Fonts.** CJK pairing (Noto Sans SC 700 display / Noto Serif SC 400 body) carries over; the eyebrow's uppercase+tracking signal weakens in CJK — pair it with the accent-line.
